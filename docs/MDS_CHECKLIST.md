...
| Diagrams                                   |      1 | Ready  | `docs/ARCHITECTURE.md` contains architecture diagrams, and `docs/UML_DIAGRAMS.md` contains GitHub-rendered UML diagrams for use case, class/domain model, activity, state, sequence, and deployment views.     |
...
| Automated tests, including agent evals     |      2 | Ready  | 56 automated tests cover Convex flows and deterministic agent quality metrics. `shared/agent-live-evals.test.ts` adds opt-in real-provider generation plus LLM-as-judge scoring; CI runs `pnpm test`.          |
