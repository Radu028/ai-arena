...
> This skill was installed by `clerk init` (or `clerk skill install`) and is pinned to clerk `1.1.1`. If `clerk --version` disagrees, refresh it with `clerk skill install` (or `bunx clerk@1.1.1 skill install`). The binary is always the source of truth, so run `clerk <command> --help` to verify anything this skill claims.
...
If that prints `1.1.1` (or any version you trust), use bare `clerk` for the rest of the session.
...
| ------------------------- | ---------------------- |
| bun (`bun.lock*`)         | `bunx clerk@1.1.1`     |
| npm (`package-lock.json`) | `npx -y clerk@1.1.1`   |
| pnpm (`pnpm-lock.yaml`)   | `pnpm dlx clerk@1.1.1` |
| yarn >= 2 (`yarn.lock`)   | `yarn dlx clerk@1.1.1` |
...
If `clerk --version` reports a newer CLI than this skill (which is pinned to `1.1.1`), run `clerk skill install` to refresh the bundled skill. The CLI binary is always the source of truth.
